...
func (s *ScheduleRepository) GetScheduleMovie(ctx context.Context, schedule *models.ScheduleStruct) ([]models.ScheduleStruct, error) {
	query := "SELECT s.id, m.title, c.cinema_name, s.date, s.time, s.location, s.price FROM schedule s JOIN cinemas c ON s.cinema_id = c.id JOIN movies m ON s.movie_id = m.id"
	...
	for rows.Next() {
		...
		err := rows.Scan(&schedule.Id, &schedule.Title, &schedule.Cinema, &schedule.Date, &schedule.Time, &schedule.Location, &schedule.Price)
		if err != nil {
			return nil, err
		}
